describe("postToken", ()=> {
    ...
    it("Creating an Address using API", () => {
      cy.getLocalStorage("access_token").should("exist");
      cy.getLocalStorage("access_token").then(token => {
...
it('Verifies through UI', function() {
    //Visit the OrnageHRM Website
    cy.visit("https://demo.spreecommerce.org/login");
    
   // Enter UserName and Password
    
    cy.get("input[placeholder='Email']").type('sharon@gmail.com')
    cy.get("input[placeholder='Password']").type('Sharon@123')
    cy.get('#new_spree_user > .btn').click();
    cy.xpath("//span[@class='account-page-user-full-name']").contains('Sharon')
    })
});